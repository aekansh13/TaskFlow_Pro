...
**MongoDB** — make sure your local MongoDB server is running, or use your Atlas URI in `.env`.

**Redis** (Windows via WSL or use Upstash for a cloud Redis):
```bash
redis-server
```
...
**Celery beat scheduler (for nightly repeat tasks):**
```bash
...
Open `frontend/index.html` in your browser (or use a simple static server like Live Server in VS Code).

## Deployment

- **Backend** → [Render](https://render.com) — set environment variables in the Render dashboard.
- **Frontend** → [Vercel](https://vercel.com) — point to the `frontend/` folder.
